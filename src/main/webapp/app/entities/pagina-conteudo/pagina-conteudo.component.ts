...
@Component({
    selector: 'jhi-pagina-conteudo',
    templateUrl: './pagina-conteudo.component.html'
})
export class PaginaConteudoComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.paginaService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<PaginaConteudo[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/pagina-conteudo'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/pagina-conteudo', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: PaginaConteudo) {
        return item.id;
    }
